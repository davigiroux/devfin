...
export default function DespesasPage() {
  ...
  const [editingDespesa, setEditingDespesa] = useState<DespesaMensal | null>(
    null
  );
  ...
  const [mesReferencia, setMesReferencia] = useState<number>(
    now.getMonth() + 1
  );
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!recorrente && (!mesReferencia || !anoReferencia)) {
        throw new Error(
          "Despesas não recorrentes devem ter mês e ano de referência"
        );
      }
  ...
  const caixaNecessario = calcularCaixaNecessario(
    despesas,
    pagamentos,
    mesView,
    anoView
      ...
      <div className="flex justify-between items-center mb-8">
        <div>
          ...
          <p className="text-muted-foreground">
            Gerencie despesas recorrentes e únicas
          </p>
        </div>
        <button
        ...
        <div className="bg-card rounded-lg shadow-sm border p-6">
          ...
          <div className="text-2xl font-bold text-foreground">
            {new Intl.NumberFormat("pt-BR", {
              style: "currency",
              currency: "BRL",
            }).format(caixaNecessario.total_despesas)}
          </div>
        </div>
        <div className="bg-card rounded-lg shadow-sm border p-6">
          ...
          <div className="text-2xl font-bold text-destructive">
            {new Intl.NumberFormat("pt-BR", {
              style: "currency",
              currency: "BRL",
            }).format(caixaNecessario.despesas_impostos)}
          </div>
        </div>
        <div className="bg-card rounded-lg shadow-sm border p-6">
          ...
          <div className="text-2xl font-bold text-warning">
            {new Intl.NumberFormat("pt-BR", {
              style: "currency",
              currency: "BRL",
            }).format(caixaNecessario.despesas_compromissos)}
          </div>
        </div>
      </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              ...
              <Select
                value={tipo}
                onChange={(e) =>
                  setTipo(e.target.value as "imposto" | "compromisso")
                }
              >
            ...
            <div>
              ...
              <CurrencyInput
                ...
                onValueChange={(value, name, values) => {
                  setValor(values?.float ?? undefined);
        ...
        {caixaNecessario.despesas.length > 0 ? (
            ...
            <table className="min-w-full divide-y divide-border">
              ...
              <tbody className="bg-card divide-y divide-border">
                {caixaNecessario.despesas.map((despesa) => (
                  <tr
                    key={despesa.id}
                    className={!despesa.ativa ? "opacity-50" : ""}
                  >
          ...
          <div className="text-center py-12">
            <p className="text-muted-foreground mb-4">
              Nenhuma despesa para {mesView}/{anoView}
            </p>
            <button
